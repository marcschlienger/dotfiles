...
dated records in =70_journal/YYYY/=, and every note starts with YAML front matter. The
commands below create notes that follow those rules, so a note made in Emacs is
...
a git repository for the same reason.
