...
Evil was removed on this branch. It had been configured with
=evil-collection=, =evil-escape=, =evil-goggles=, =evil-surround=,
=evil-commentary=, =evil-tex= and =general= for a leader key --- and at
the same time seven modes were pushed into =evil-emacs-state-modes=,
including =dired-mode= and =eshell-mode=, which is precisely what
=evil-collection= exists to handle. Paying for an emulation layer and
then opting out of it where it matters most is the "feels like a hack"
feeling made concrete.

Only three leader bindings had ever been written, so the configuration
cost of leaving is nil; the cost is muscle memory.

What each package did, and what now does it:

| package          | replacement                                       |
|------------------+---------------------------------------------------|
| evil-commentary  | =M-;= (~comment-dwim~), built in and better       |
| evil-tex         | AUCTeX's =C-c C-e= / =C-c C-m=, plus cdlatex      |
| evil-surround    | =M-(= and friends; Paredit in Lisp buffers        |
| evil-goggles     | =pulse.el=, built in                              |
| goto-chg         | =C-u C-SPC= through the mark ring                 |
| general          | plain ~global-set-key~ and use-package's =:bind=  |
| evil-escape      | not needed; it was orphaned upstream anyway       |

=ms-toggle-line-number-type= survives in [[*Basic settings]]; it lost its
=SPC n= binding and is now an ~M-x~ command.

The Evil packages are deliberately *not* uninstalled. =elpa/= is ignored
by git and shared between branches, so removing them would break =main=
as well. Uninstall them only if this branch is merged.
