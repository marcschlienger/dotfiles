...
Wiki links are resolved while a buffer is fontified: markdown-mode looks up the target
file of every =[[link]]= it draws, by searching the whole project. Outside a version
controlled directory that search asks for a project root in the middle of redisplay, so
the settings apply only to buffers visiting a file inside the vault. The vault must stay
a git repository for the same reason. The commands themselves live in
=lib.d/lib-notes.el=.

#+begin_src emacs-lisp :tangle init.d/init-notes.el :mkdirp yes
...
** The `lib-notes.el' library

The notes vault in =~/Notes= is a directory of Markdown files shared with other Markdown
editors. Filenames are lowercase ASCII in kebab case, living notes sit in a domain folder,
dated records in =70_journal/YYYY/=, and every note starts with YAML front matter. These
commands create notes that follow those rules, so a note made in Emacs is
indistinguishable from one made elsewhere.

A note captured elsewhere lands in =00_inbox/= under a timestamp name. =ms-notes-file=
files it: it takes the title from the note's heading, or from its filename when that is
not a timestamp, renames it to the slug and moves it into its domain's folder, or into
the journal for a meeting. Emacs does not rewrite links to a renamed file, so the command
refuses to rename a note that another note already links to.

#+begin_src emacs-lisp :tangle lib.d/lib-notes.el :mkdirp yes
