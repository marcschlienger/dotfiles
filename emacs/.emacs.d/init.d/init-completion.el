...
;; from a `with-eval-after-load' in corfu.el, and this guard does prevent
;; that: a normal session on 31 loads neither the package nor the warning.
;;
;; It does not prevent installation.  use-package acts on `:ensure' while
;; the macro is being byte-compiled, ahead of any `when' around it, so
;; byte-compiling this file on 31 fetches corfu-terminal and popon and
;; leaves them sitting unused in `package-user-dir'.
