...
the buffer -- saving still works where the server is not installed."
  (when (and (fboundp 'eglot-managed-p) (eglot-managed-p))
    ;; Organizing imports signals when the server offers no such action,
    ;; which for a file whose imports are already correct is the ordinary
    ;; case.  A `before-save-hook' must not refuse the save over that.
    (ignore-errors
...
(defun ms/go-mode-setup ()
  ...
  (add-hook 'before-save-hook #'ms/go-format-on-save nil t))
