...
(use-package treesit
  ...
  (defun ms/treesit-activate-remappings ()
    "Pick up every tree-sitter grammar that is available locally.
Run this after installing a grammar to use it in the current session
without restarting."
    (interactive)
...
;;; Go
;; Installed for its `auto-mode-alist' entry and as the mode `go-ts-mode' is
;; remapped from; the remapping in the tree-sitter block above does the rest.
;; On a machine where the grammar has not been built this is what .go falls
;; back to.
(defun ms/go-format-on-save ()
  "Organize imports and reformat the buffer through the language server.
Both are server operations, so this does nothing when no server is managing
the buffer -- saving still works where the server is not installed."
  (when (and (fboundp 'eglot-managed-p) (eglot-managed-p))
    ;; A file whose imports are already correct offers no organizing action,
    ;; and Eglot signals for that.  The save itself is not at risk -- Emacs
    ;; runs `before-save-hook' inside `with-demoted-errors' -- but a signal
    ;; here would report an error on nearly every save and skip the
    ;; reformatting below, which is the next form in this same function.
    ;;
    ;; Eglot raises everything as a plain `error', so that ordinary case can
    ;; only be told apart by its message.  Anything else -- a timeout, a
    ;; server that died mid-request -- is worth hearing about.
    (condition-case error-data
        (eglot-code-action-organize-imports (point-min) (point-max))
      (error
       (let ((text (error-message-string error-data)))
         (unless (string-search "No \"source.organizeImports\" code actions here"
                                text)
           (display-warning 'eglot
                            (format "Organizing imports failed: %s" text)
                            :warning)))))
    (eglot-format-buffer)))

(defun ms/go-mode-setup ()
  "Reformat Go buffers through the language server on save.
The depth puts this ahead of Eglot's own `willSave' notification, which
`eglot--managed-mode' adds at the default depth once it connects: the
server should hear about the save after these edits, not before."
  (add-hook 'before-save-hook #'ms/go-format-on-save -10 t))

(use-package go-mode
  :ensure t
  :defer t
  :hook ((go-mode . ms/go-mode-setup)
         (go-ts-mode . ms/go-mode-setup)))
