...
;;
;; Enchant does not check spelling itself.  It dispatches to whichever
;; provider it was built with, and that differs by platform:
;;
;;   macOS   Homebrew's enchant ships only aspell and AppleSpell.  AppleSpell
;;           offers neither en_US nor de_DE, so both come from aspell, and the
;;           hunspell dictionaries installed elsewhere are unreachable here.
;;           Aspell's German does not decompose compounds, so ordinary words
;;           -- Hausaufgabenbetreuung, Klassenarbeitstermin -- are flagged.
;;   Debian  a different provider set, possibly including hunspell.
;;
