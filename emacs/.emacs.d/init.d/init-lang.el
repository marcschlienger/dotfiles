...
(use-package treesit
  ...
  ;; is not installed; the association is Emacs's own business.  How it does
  ;; that differs by version, which is why it is not relied on:
  ;;
  ;;   31  go-ts-mode.el autoloads a `go-ts-mode-maybe' dispatcher into
  ;;       `auto-mode-alist' unconditionally.
  ;;   30  no dispatcher exists; the association comes from an autoloaded
  ;;       form guarded by `(treesit-ready-p 'go)', evaluated when the
  ;;       autoloads are rather than when the grammar is built.
  ;;
  ;; `ms/treesit-associate-go' below settles both cases: it claims .go only
  ;; when the grammar is present and nothing else has claimed it, so it is a
  ;; no-op on 31 and a repair on 30.
  ...
  :config
  (defun ms/treesit-associate-go ()
    "Make sure .go files reach `go-ts-mode'.
Does nothing when the grammar is missing, and nothing when something has
already claimed the extension -- so it repairs Emacs 30 without displacing
the dispatcher Emacs 31 installs."
    (when (and (treesit-language-available-p 'go)
               (not (assoc-default "x.go" auto-mode-alist #'string-match)))
      (add-to-list 'auto-mode-alist '("\\.go\\'" . go-ts-mode))))

  (defun ms/treesit-activate-remappings ()
    "Pick up every tree-sitter grammar that is available locally.
Remaps the modes that have a traditional counterpart, and claims .go,
which needs an association rather than a remapping because `go-mode' is
not installed.  Run this after installing a grammar to use it in the
current session without restarting."
    ...
    ;; Go is not in the remapping list, so it has to be picked up here too --
    ;; otherwise installing the grammar in a running session left .go
    ;; unassociated until the next restart.
    (ms/treesit-associate-go))
